spectraListRoundingFiltering <- function(specScanXIC, spectraList, roundingDigit = 1) {
  ...
  selectIndex <- round(spectraList.mat[, 1], digits = roundingDigit) %in% unique(round(specScanXIC[, 1], digits = roundingDigit))
  ...
  ##
  xDiff <- which(abs(diff(spectraList.mat.sb[, 3])) > 0)
  #
