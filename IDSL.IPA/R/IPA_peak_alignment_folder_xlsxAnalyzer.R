IPA_peak_alignment_folder_xlsxAnalyzer <- function(PARAM, PARAM_ID, checkpoint_parameter, correctedRTcheck = FALSE, CSAcheck = FALSE, allowedVerbose = TRUE) {
  ...
  if (length(xPARAM_ID) == 0) {
    if (allowedVerbose) {print(paste0("ERROR!!! Problem with ", PARAM_ID, "!"))}
  ...
  if (checkpoint_parameter) {
    ...
    if (tolower(peak_alignment_folder) != "na") {
      ...
      if (!dir.exists(peak_alignment_folder)) {
        if (allowedVerbose) {print(paste0("ERROR!!! Problem with ", PARAM_ID, "! Please make sure the full path is provided!"))}
        checkpoint_parameter <- FALSE
      } else {
        peakXcol_FN <- paste0(peak_alignment_folder, "/peakXcol.Rdata")
        ##
        if (!file.exists(peakXcol_FN)) {
          if (allowedVerbose) {print(paste0("ERROR!!! Problem with ", PARAM_ID, "! `peakXcol.Rdata` was not detected in the `peak_alignment` folder (", PARAM_ID, ")!"))}
          checkpoint_parameter <- FALSE
        }
        ##
        ...
        if (!file.exists(peak_height_FN)) {
          if (allowedVerbose) {print(paste0("ERROR!!! Problem with ", PARAM_ID, "! `peak_height.Rdata` was not detected in the `peak_alignment` folder (", PARAM_ID, ")!"))}
          checkpoint_parameter <- FALSE
        }
        ##
        ...
        if (!file.exists(peak_R13C_FN)) {
          if (allowedVerbose) {print(paste0("ERROR!!! Problem with ", PARAM_ID, "! `peak_R13C.Rdata` was not detected in the `peak_alignment` folder (", PARAM_ID, ")!"))}
          checkpoint_parameter <- FALSE
        }
        ##
        if (CSAcheck) {
          ...
          if (!file.exists(alignedPeakHeightTableCorrelationList_FN)) {
            if (allowedVerbose) {print(paste0("ERROR!!! Problem with ", PARAM_ID, "! `alignedPeakHeightTableCorrelationList.Rdata` was not detected in the `peak_alignment` folder (", PARAM_ID, ")!"))}
            checkpoint_parameter <- FALSE
          }
        }
        ##
        if (correctedRTcheck) {
          ...
          if (!file.exists(listCorrectedRTpeaklists_FN)) {
            if (allowedVerbose) {print(paste0("ERROR!!! Problem with ", PARAM_ID, "! `listCorrectedRTpeaklists.Rdata` was not detected in the `peak_alignment` folder (", PARAM_ID, ")!"))}
            checkpoint_parameter <- FALSE
          }
        }
      }
